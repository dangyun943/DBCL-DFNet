...
class HeteroDataset:
    ...
    def _create_hetero_data(self, omics_idx: int, feat_removal_idx: int = None):
        ...
        if self.tune_hyperparameters:
            ...
            hetero_data['patient'].train_mask = torch.tensor(np.arange(single_data.num_samples) < single_data.num_train_samples, dtype=torch.bool)
        ...
        assert belong_to.max() < hetero_data['patient'].x.shape[0], \
            f"目标节点索引越界：最大值 {belong_to.max()}，合法范围 [0, {hetero_data['patient'].x.shape[0]})"
