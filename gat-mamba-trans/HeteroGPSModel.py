...
class HeteroGPSModel(torch.nn.Module):
    def __init__(self, hidden_channels: List[int], out_channels, num_layers, num_heads: int = 1, bias: bool = True,
        ...
        conv = HeteroConv({
            ('patient', 'similar', 'patient'): PhaseAgnosticGAT(-1, hidden_channels[0], heads=num_heads, concat=False,
                                                       dropout=dropout, add_self_loops=True, edge_dim=1),
            ('feature', 'similar', 'feature'): PhaseAgnosticGAT(-1, hidden_channels[0], heads=num_heads, concat=False,
                                                       dropout=dropout, add_self_loops=True, edge_dim=1),  ##edge=2
            ('feature', 'belong', 'patient'): PhaseAgnosticGAT((-1, -1), hidden_channels[0], heads=num_heads,
                                                      concat=False, dropout=dropout, add_self_loops=False,
                                                      edge_dim=None),
        }, aggr='mean')
        self.convs.append(conv)
        for layer_id in range(num_layers):
            ...
            conv = HeteroConv({
                ('feature', 'belong', 'patient'): PhaseAgnosticGAT((-1, -1), hidden_channels[layer_id], heads=num_heads,
                                                          concat=False, dropout=dropout, add_self_loops=False,
                                                          edge_dim=None),
                ('patient', 'similar', 'patient'): HeteroGPSLayer(
                    dim_h=hidden_channels[layer_id],
                    local_gnn_type="GAT",
                ...
                ('feature', 'similar', 'feature'): HeteroGPSLayer(
                    dim_h=hidden_channels[layer_id],
                    local_gnn_type="GAT",
                    global_model_type="Mamba",
                    num_heads=num_heads,
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict, phase="train", stage="only"):
        for i, conv in enumerate(self.convs):
            phase_dict = {
            ('patient', 'similar', 'patient'): phase,
            ('feature', 'similar', 'feature'): phase,
            ('feature', 'belong', 'patient'): phase
            }
            x_dict = conv(
                x_dict,
                edge_index_dict,
            ...
            if 'patient' in x_dict:
                x_dict['patient'] = F.leaky_relu(x_dict['patient'], negative_slope=0.2)
            if 'feature' in x_dict:
                x_dict['feature'] = F.gelu(x_dict['feature'])

        return x_dict['patient'], self.lin(x_dict['patient'])
...
class HeteroGPSLayer(nn.Module):
    """Local MPNN + full graph attention x-former layer.
    """

    def __init__(self, dim_h,inedge,
        ...
        if local_gnn_type == 'None':
            ...
            self.local_model = GatedGCNLayer(in_dim=dim_h, out_dim=dim_h,
                                             dropout=dropout,
                                             inedge=self.inedge,
                                             residual=True,
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict,phase):
        # 构造同构图Batch进行处理
        ...
        num_nodes = batch.num_nodes  # 如果存在num_nodes属性
        batch.batch = torch.ones(num_nodes, dtype=torch.long)
        h = batch.x
        ...
        if self.self_attn is not None:
            ...
            elif self.global_model_type == 'Mamba_Degree':   ##不行
            ...
            #         h_attn = sum(mamba_arr) / 5
                ...
                aug_idx, aug_mask = augment_seq(batch.edge_index, batch.batch, 10)
