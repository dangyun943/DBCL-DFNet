...
class HeteroDataset:
    def __init__(self, raw_data1,raw_data, sparsity_rate: float, tune_hyperparameters: bool = False, seed: int = 24):
    ...
    def _create_hetero_data(self, omics_idx: int, feat_removal_idx: int = None):
        hetero_data = HeteroData().to(device)
        single_data = self.raw_data.get(omics_idx)
        single_data1 = self.raw_data1.get(omics_idx)
        sparse_adj_mat, sparse_feat_df, node_relevance, selected_subset = single_data.get_data_structure()
        from scipy.sparse import csr_matrix
        sparse_feat_corr = csr_matrix(sparse_feat_df.values)
        ...
        data1 = pd.concat([single_data1.train_data, single_data1.test_data], axis=0)
        hetero_data['patient'].x1= torch.tensor(data1.values, dtype=torch.float).to(device)
        ...
        hetero_data['patient'].x = torch.tensor(data.values, dtype=torch.float).to(device)
        ...
        hetero_data['patient', 'similar', 'patient'].edge_index = torch.tensor(np.concatenate(patient_adj_mat.nonzero()).flatten().reshape(2, -1), dtype=torch.long).to(device)
        hetero_data['patient', 'similar', 'patient'].edge_attr = torch.tensor(patient_adj_mat.data.reshape(-1, 1), dtype=torch.float).to(device)  # [num_edges_similar, 1]
        ...
        feature_edge_attr = feature_edge_attr_corr  # 直接使用相关性
        ...
        extracted_feat_info = node_relevance.values.reshape(-1, 1)
        ...
        hetero_data['feature'].x = feat_x_new.to(device)
        hetero_data['feature'].feat_name = data.columns.tolist()
        hetero_data['feature', 'similar', 'feature'].edge_index = torch.tensor(feature_edge_index, dtype=torch.long).to(device)
        feature_edge_attr = feature_edge_attr.A
        hetero_data['feature', 'similar', 'feature'].edge_attr = torch.tensor(feature_edge_attr, dtype=torch.float).to(device)
        ...
        hetero_data['feature', 'belong', 'patient'].edge_index = torch.tensor(belong, dtype=torch.long).to(device)
